...
list_node * emmited_quads_temp = NULL;
list_node * curr_emmited_quads_temp = NULL;
...
void backpatch(list_node * head,unsigned int quadid){
	list_node * temp = head;
	while(temp){
		patch_label(temp->value,quadid);
		temp = temp->next;
	}
} 
...
void backpatch_jumps(unsigned int start, unsigned int end,unsigned int value){
	unsigned int i,j,entered;
	list_node * temp;
	entered = 0;
	for(i=start;i<=end;i++){
		if(quads[i].op==jump && i>0 && is_if_intr(quads[i-1].op)){
 
			if(quads[i+1].next_if_type == or_case){
				patch_label(i,i+1);
			}
			else{
				...
				for(j=i+1;j<=end;j++){
					...
					if(is_if_intr(quads[j].op) && quads[j].next_if_type==or_case && quads[j].if_level<quads[i-1].if_level){
						patch_label(i,j);
						entered = 1;
						break;
					}
					if(!entered)patch_label(i,value);
				}
...
void backpatch_if(unsigned int start, unsigned int end,unsigned int value,unsigned int offset){
	unsigned int i;
	for(i=start;i<=end;i++){
		if(is_if_intr(quads[i].op)){
				if(i+2<=end && is_if_intr(quads[i+2].op)){

					if(quads[i+2].next_if_type == or_case){
						if(quads[i].type==and_case)patch_label(i,end+1+offset);
						else patch_label(i,end+1+offset);
					}
					else{
						if(quads[i].type==and_case)patch_label(i,i+2 );
						else patch_label(i,i+2 );
					}
				}
		}
	}
...
void special_if_backpatch(unsigned int start, unsigned int end, iftype it){

}

void setIfType(iftype it,unsigned int index){
	quads[index].type = it;
}

unsigned int is_if_intr(opcode op){
	return(op>=if_eq && op<=if_greater);
}

char * print_if_type(iftype it){
	if(it==and_case)return "end_case";
	return "or_case";
}